...
esp_err_t es8311_codec_init(void) {
  ESP_LOGI(TAG_AUDIO, "Initializing ES8311 codec...");
  ...
  if (i2c_mutex != NULL && xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
    ...
    if (error != 0) {
      ESP_LOGE(TAG_AUDIO, "ES8311 not found at I2C address 0x18");
      return ESP_FAIL;
    }
  } else {
    ESP_LOGE(TAG_AUDIO, "Failed to acquire I2C mutex");
    return ESP_FAIL;
  }
  
  es8311_handle_t es_handle = es8311_create(0, ES8311_ADDRRES_0);
  if (!es_handle) {
    ESP_LOGE(TAG_AUDIO, "ES8311 create failed");
    return ESP_FAIL;
  }
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG_AUDIO, "Sample frequency config failed: %s", esp_err_to_name(err));
    return err;
  }
  
  err = es8311_microphone_config(es_handle, false);
  if (err != ESP_OK) {
    ESP_LOGE(TAG_AUDIO, "Microphone config failed: %s", esp_err_to_name(err));
    return err;
  }
  
  err = es8311_voice_volume_set(es_handle, EXAMPLE_VOICE_VOLUME, NULL);
  if (err != ESP_OK) {
    ESP_LOGE(TAG_AUDIO, "Volume set failed: %s", esp_err_to_name(err));
    return err;
  }
  
  err = es8311_microphone_gain_set(es_handle, EXAMPLE_MIC_GAIN);
  if (err != ESP_OK) {
    ESP_LOGE(TAG_AUDIO, "Microphone gain set failed: %s", esp_err_to_name(err));
    return err;
  }
...
bool playWavFromSDCard(const char* wavFile, bool* continueFlag, bool isTestMode = false) {
    const size_t CHUNK_SIZE = 2048;
    int32_t wavDataSize = memory->getWavDataSize(wavFile);
    
    if (wavDataSize <= 0) {
        ESP_LOGE(TAG_AUDIO, "Invalid WAV file: %s", wavFile);
        return false;
    }
    
    ESP_LOGI(TAG_AUDIO, "Playing: %s (%d bytes)", wavFile, wavDataSize);
    
    uint8_t* audioBuffer = (uint8_t*)malloc(CHUNK_SIZE);
    int16_t* stereoBuffer = (int16_t*)malloc(CHUNK_SIZE * 2);
    ...
    while (*continueFlag && offset < (size_t)wavDataSize) {
        size_t bytesToRead = min((size_t)(wavDataSize - offset), CHUNK_SIZE);
        ...
        if (bytesRead > 0) {
            ...
            ESP_LOGW(TAG_AUDIO, "Read error at offset %d", offset);
            success = false;
            break;
        }
    ...
    if (success) {
        ESP_LOGI(TAG_AUDIO, "Playback complete (%d bytes)", totalWritten);
    }
    
    return success;
}

void playDefaultAudio(bool* continueFlag) {
    const size_t CHUNK_SIZE = 2048;
    size_t offset = 0;
    
    ESP_LOGI(TAG_AUDIO, "Playing default audio (%d bytes)", canon_pcm_len);
    
    while (*continueFlag && offset < canon_pcm_len) {
        size_t bytesToWrite = min((size_t)(canon_pcm_len - offset), CHUNK_SIZE);
...
void controlAlarmTask(void *param) {
    ...
    ESP_LOGI(TAG_AUDIO, "Alarm task started, waiting for system ready...");
    ...
    if (i2s.begin(I2S_MODE_STD, EXAMPLE_SAMPLE_RATE, I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO, I2S_STD_SLOT_BOTH)) {
        ESP_LOGI(TAG_AUDIO, "I2S initialized");
        ...
        if (codec_result == ESP_OK) {
            ...
            // Test tone
            const int testDuration = EXAMPLE_SAMPLE_RATE / 10;
            ...
            if (testTone) {
                ...
                i2s.write((uint8_t*)testTone, testDuration * 2 * sizeof(int16_t));
                free(testTone);
                vTaskDelay(pdMS_TO_TICKS(200));
            }
        } else {
            ESP_LOGW(TAG_AUDIO, "Codec init failed, continuing anyway");
            audioInitialized = true;
        }
    }
    ...
    while (true) {
        ...
        if (isPlay && !isPlaying && audioInitialized) {
            ...
            playWavFromSDCard(wavFilePath.c_str(), &isPlay, true);
        ...
        if (shouldBePlaying && !isPlaying && audioInitialized) {
            ...
            ESP_LOGI(TAG_AUDIO, "Alarm triggered (Schedule: %s - %s)",
            ...
            if (selectedFile != "") {
                ...
                playedSuccessfully = playWavFromSDCard(wavFilePath.c_str(), &isAlarm);
            }
            
            if (!playedSuccessfully) {
                playDefaultAudio(&isAlarm);
            }
